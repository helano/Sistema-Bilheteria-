                ...
                <div class="container">
                    <h1> Tabela de Jogos </h1>
                        ...
                        <table border = 1 class="table table-condensed" Style="margin:20px;" >
                            <tr> <th> Mandante </th> <th> Visitante </th> <th> Competição </th><th>Estádio </th> <th>Data</th> </tr>
                <%
                    ...
                    try{
                        ...
                        rs = st.executeQuery("SELECT (select nome_comp from competicao  where partida_torcedor.id_competicao = competicao.id_competicao) as competicao,(select nome_time from timer  where partida_torcedor.id_time = timer.id_time) as timer,(select nome_time from timer  where partida_torcedor.id_time2 = timer.id_time) as timer, (select nome_est from estadio where partida_torcedor.id_estadio = estadio.id_estadio) as estadio,partida_torcedor.data, partida_torcedor.id_partida_torcedor FROM partida_torcedor WHERE partida_torcedor.id_torcedor="+codigo_torcedor+";");
                       ...
                       data[i]=rs.getString(5);
                               ...
                               <tr class="<%=time1[i]%>"> <td> <%=time1[i]%> </td> <td > <%=time2[i] %> </td> <td > <%=competicao[i] %> </td> <td > <%=estadio[i] %> </td> <td > <%=data[i] %>  </tr>
